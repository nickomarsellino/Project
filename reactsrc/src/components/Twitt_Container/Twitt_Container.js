...
class Twitt_Container extends Component {
  ...
  getData() {
    axios.get('/api/users/tweets').then(res => {
      this.setState({tweetData: res.data});
      console.log("tweetData ", this.state.tweetData);
    });
  }

  openModalTweet(tweetId) {
    axios.get('http://localhost:3000/api/users/tweet/' + tweetId).then(res => {
      this.setState({tweet: res.data, modalTweet: true});
    });
  }

  openModalDelete(tweetId) {
    axios.get('http://localhost:3000/api/users/tweet/' + tweetId).then(res => {
      this.setState({tweet: res.data, modalDelete: true});
    });
  }
  ...
  closeModalTweet(isOpen) {
    if (isOpen) {
      {
        this.setState({modalTweet: false})
      }
    }
  }
  ...
  render() {
    return (<div>
      {
        this.state.tweetData.map(tweet => <Card className="Tweet_Container">
          <CardBody className="Tweet">
            <Feed>
              <Feed.Event onClick={() => this.openModalTweet(tweet._id)}>
                <Feed.Label image={profile} style={{
                    width: "10%",
                    padding: "5px 0"
                  }}/>
                <Feed.Content>
                  ...
                  <Feed.Extra text="text" content={tweet.tweetText}/>
                  <br/>
                  <Feed.Date content={<Timestamp time = {
                      tweet.timestamp
                    }
                    precision = {
                      1
      ...
      <Modal_Twitt isOpen={this.state.modalTweet} tweet={this.state.tweet} isClose={this.closeModalTweet}/>

      <Modal_Delete isOpen={this.state.modalDelete} tweet={this.state.tweet} isClose={this.closeModalDelete}/>
